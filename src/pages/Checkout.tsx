          ...
          {steps.map((s, i) => (
            <div key={s.id} className="flex items-center gap-2">
              <div className="flex items-center gap-2">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-colors ${i <= stepIndex ? 'bg-gold text-luxury-black' : 'bg-gray-200 text-white'}`}>
                  {i + 1}
                </div>
                <span className={`font-sans font-light text-[10px] tracking-widest uppercase ${i === stepIndex ? 'text-luxury-black' : 'text-luxury-lightgray'}`}>
                  {s.label}
                </span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-6 md:p-8 border border-gray-100">
            ...
            {step === 'payment' && (
              <div className="space-y-6 animate-fade-in">
                ...
                {/* Bank transfer — única opción */}
                <div className="border border-gold bg-gold/5 p-5 flex items-start gap-4">
                  <Building2 size={20} strokeWidth={1.5} className="text-gold flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="font-sans font-light text-sm text-luxury-black mb-1">Transferencia bancaria</p>
                    <p className="font-sans font-light text-[10px] text-luxury-lightgray">CBU / CVU / Alias</p>
                  </div>
                </div>

                <div className="space-y-2">
                  <p className="font-sans font-light text-[10px] tracking-widest uppercase text-luxury-lightgray mb-3">Datos bancarios</p>
                  <div className="bg-cream p-4 space-y-2">
                    <Row label="Banco" value="Banco Galicia" />
                    <Row label="Titular" value="DE LEPARFUM S.A." />
                    <Row label="CBU" value="0070123530000000100012" />
                    <Row label="Alias" value="DELEPARFUM.TIENDA" />
                    <Row label="CUIT" value="30-71234567-8" />
                  </div>
                  <p className="font-sans font-light text-[10px] text-luxury-lightgray leading-relaxed pt-1">
                    Una vez realizada la transferencia, te vamos a contactar por WhatsApp para confirmar el pago y coordinar el envío.
                  </p>
                </div>
            ...
            <div className="flex justify-between mt-8 pt-6 border-t border-gray-100">
              {stepIndex > 0 ? (
                <button onClick={() => setStep(steps[stepIndex - 1].id)} className="btn-secondary text-xs px-6">Atrás</button>
              ) : (
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border border-gray-100 p-6 sticky top-24">
              <h3 className="font-serif text-base text-luxury-black mb-5 pb-4 border-b border-gray-100">Tu Pedido</h3>
